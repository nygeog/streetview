...
for buf in bufs:
    for crashtype in crashtypes:
        for i in years:
            ...
            inFeat = pd+'/input/rated_intersections/'+buf+'.shp'
            zoneField = 'svsuid'
            valueRaster = pd+'/processing/kd.gdb/crashes_'+crashtype+'_'+i_s+'_kd'
            workspace = pd+'/processing/kd_ws/'
            zoneName = 'zone'
            numDivides = 4
            ...
            scratchName = arcpy.CreateUniqueName("Scratch"+buf+crashtype+i_s+".gdb", workspace)

            # Get the foldername from the scratchName (entire path)
            scratchFldr = os.path.split(scratchName)[1]

            # Create the folder.
            arcpy.CreateFileGDB_management(workspace, scratchFldr)
