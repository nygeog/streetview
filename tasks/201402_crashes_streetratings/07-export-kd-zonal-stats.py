...
for buf in bufs:
	for crashtype in crashtypes:
		for i in range(1995, 2014):
			i_s  = str(i)
			table   = pd+'/processing/zonal_stats.gdb/'+buf+'_crashes_'+crashtype+'_'+i_s+'_zon'
			...
			with open(outfile,'wb') as f:
			    ...
			    del row
			print buf+'_'+crashtype + " raw export is complete"

print 'export all geos to csv at:' +  time.strftime('%c') + ' - 1 minute'
for buf in bufs:
	for crashtype in crashtypes:
		table   = pd+'/processing/zonal_stats.gdb/'+buf+'_crashes_'+crashtype+'_1995_2013_zon'
